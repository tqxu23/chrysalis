from models.components.insitu.INASCostCore.microbench_measurements import predict_dma_energy, predict_leavecmac_energy, predict_mathop_energy, \
    predict_latency
...
class PlatformCostModel:
    ...
    PLAT_MSP430_EXTNVM = {
        ...
        "E_FD_W_OVHD": (E_ADD * (5 + 3)) + (E_MUL * (6 + 3)),
        # according to asm: (E_ADD * (7+3)) + (E_MUL * (6+3)),
        "E_FD_O_RUI_OVHD": (E_ADD * (5 + 2)) + (E_MUL * (2 + 3)),
        # according to asm: (E_ADD * (5+2)) + (E_MUL * (2+3)),
        "E_FD_O_RUW_OVHD": (E_ADD * (5 + 2)) + (E_MUL * (2 + 2)),
        # according to asm: (E_ADD * (5+2)) + (E_MUL * (2+2)),
        "E_FD_O_RUO_OVHD": (E_ADD * (5 + 2)) + (E_MUL * (2 + 2)),
        # according to asm: (E_ADD * (5+2)) + (E_MUL * (2+2)),
        "E_BD_O_RUI_OVHD": (E_ADD * (4 + 5)) + (E_MUL * (6 + 2)),
        # according to asm: (E_ADD * (4+5)) + (E_MUL * (6+2)),
        "E_BD_O_RUW_OVHD": (E_ADD * (4 + 7)) + (E_MUL * (4 + 5)),
        # according to asm: (E_ADD * (4+7)) + (E_MUL * (4+5)),
        "E_BD_O_RUO_OVHD": (E_ADD * (2 + 5)) + (E_MUL * (3 + 2)),
        # according to asm: (E_ADD * (2+5)) + (E_MUL * (3+2)),

        # vector MAC compute cost
        "E_OP_VECMAC": _proxy_leavecmac_energy,
        # computation invocation overhead (addressing)
        "E_OP_PSUM_OVHD": (E_ADD * (4 + 3)) + (E_MUL * (2 + 3)),
        # according to asm: (E_ADD * (4+3)) + (E_MUL * (2+3)),
        ...
        "L_FD_O_RUI_OVHD": (E_ADD * (5 + 2)) + (E_MUL * (2 + 3)),
        # according to asm: (L_ADD * (5+2)) + (L_MUL * (2+3)),
        "L_FD_O_RUW_OVHD": (E_ADD * (5 + 2)) + (E_MUL * (2 + 2)),
        # according to asm: (L_ADD * (5+2)) + (L_MUL * (2+2)),
        "L_FD_O_RUO_OVHD": (E_ADD * (5 + 2)) + (E_MUL * (2 + 2)),
        # according to asm: (L_ADD * (5+2)) + (L_MUL * (2+2)),
        "L_BD_O_RUI_OVHD": (E_ADD * (4 + 5)) + (E_MUL * (6 + 2)),
        # according to asm: (L_ADD * (4+5)) + (L_MUL * (6+2)),
        "L_BD_O_RUW_OVHD": (E_ADD * (4 + 7)) + (E_MUL * (4 + 5)),
        # according to asm: (L_ADD * (4+7)) + (L_MUL * (4+5)),
        "L_BD_O_RUO_OVHD": (E_ADD * (2 + 5)) + (E_MUL * (3 + 2)),
        # according to asm: (L_ADD * (2+5)) + (L_MUL * (3+2)),
