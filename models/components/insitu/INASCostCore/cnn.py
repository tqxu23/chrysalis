...
from models.components.insitu.INASCostCore.conv_tiled import est_cost_CONV_powcycle_intpow, est_cost_CONV_contpow, est_cost_CONV_reboot_contpow
from models.components.insitu.INASCostCore.pool_tiled import est_cost_POOL_powcycle_intpow, est_cost_POOL_contpow
from models.components.insitu.INASCostCore.capacitor import cal_cap_recharge_time_custom, cap_energy
...
def est_cost_layer_intpow(layer, params_exec, params_pres, plat_settings, plat_cost_profile):
    ...
    if layer_type == "CONV":
        if (common.check_conv(layer)):
            E_rb, L_rb, E_fd_ngt0, L_fd_ngt0, E_fd_n0, L_fd_n0, E_fl, L_fl, E_cp, L_cp, E_bd, L_bd, E_bl, L_bl = est_cost_CONV_powcycle_intpow(params_exec, params_pres, plat_settings, plat_cost_profile, exclude_rb_cost=False, return_breakdown=True)
            Epc_max = E_rb + E_fd_ngt0 + E_fl + E_cp + E_bd + E_bl
            Lpc_max = L_rb + L_fd_ngt0 + L_fl + L_cp + L_bd + L_bl
            Epc_min = E_rb + E_fd_n0 + E_fl + E_cp + E_bd + E_bl
            Lpc_min = L_rb + L_fd_n0 + L_fl + L_cp + L_bd + L_bl
            Eb = E_rb
            Lb = L_rb
            Er = E_fd_ngt0+E_fl
            Lr = L_fd_ngt0+L_fl
        ...
        if (common.check_pool(layer)):
            ...
            Epc_min = E_rb + E_fd + E_fl + E_cp + E_bd + E_bl
            Lpc_min = L_rb + L_fd + L_fl + L_cp + L_bd + L_bl
            Eb = E_rb
            Lb = L_rb
            Er = E_fd+E_fl
            Lr = L_fd+L_fl
    ...
    return npc, npc_n0, npc_ngt0, Epc_max, Lpc_max, Epc_min, Lpc_min, Eb, Lb, Er, Lr, Ec, Lc, Ew, Lw
