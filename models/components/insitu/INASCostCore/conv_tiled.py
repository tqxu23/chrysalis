...
# local imports
from models.components.insitu.INASCostCore import common
...
def est_cost_CONV_layerinputfetch_contpow(layer, params_exec, plat_cost_profile):
    ...
    er_Tn = plat_cost_profile['E_DMA_NVM_TO_VM'](Tn);
    lr_Tn = plat_cost_profile['L_DMA_NVM_TO_VM'](Tn)
    er_Tm = plat_cost_profile['E_DMA_NVM_TO_VM'](Tm);
    lr_Tm = plat_cost_profile['L_DMA_NVM_TO_VM'](Tm)
    # energy/latency overhead of each transfer
    eofI = plat_cost_profile['E_FD_I_OVHD'];
    lofI = plat_cost_profile['L_FD_I_OVHD']
    eofW = plat_cost_profile['E_FD_W_OVHD'];
    lofW = plat_cost_profile['L_FD_W_OVHD']

    eofO_ruI = plat_cost_profile['E_FD_O_RUI_OVHD'];
    lofO_ruI = plat_cost_profile['L_FD_O_RUI_OVHD']
    eofO_ruW = plat_cost_profile['E_FD_O_RUW_OVHD'];
    lofO_ruW = plat_cost_profile['L_FD_O_RUW_OVHD']
    eofO_ruO = plat_cost_profile['E_FD_O_RUO_OVHD'];
    lofO_ruO = plat_cost_profile['L_FD_O_RUO_OVHD']

    # -- calc energy cost depending on reuse scheme
    if inter_lo == "reuse_I":
        total_en_cost = ((Rtr * Ctc * Ntn) * (nI * (er_Tn + eofI))) + ((num_tiles) * (nW * (er_Tn + eofW))) + (
                    ngt0 * (nO * (er_Tm + eofO_ruI)))
        total_lat_cost = ((Rtr * Ctc * Ntn) * (nI * (lr_Tn + lofI))) + ((num_tiles) * (nW * (lr_Tn + lofW))) + (
                    ngt0 * (nO * (lr_Tm + lofO_ruI)))

    elif inter_lo == "reuse_W":
        total_en_cost = ((num_tiles) * (nI * (er_Tn + eofI))) + ((Mtm * Ntn) * (nW * (er_Tn + eofW))) + (
                    ngt0 * (nO * (er_Tm + eofO_ruW)))
        total_lat_cost = ((num_tiles) * (nI * (lr_Tn + lofI))) + ((Mtm * Ntn) * (nW * (lr_Tn + lofW))) + (
                    ngt0 * (nO * (lr_Tm + lofO_ruW)))
...
def est_cost_CONV_layeroutputbackup_contpow(layer, params_exec, plat_cost_profile):
    ...
    tile_en_cost, tile_lat_cost = est_cost_CONV_tileoutputbackup_intpow(params_exec, params_pres,
                                                                        plat_cost_profile)  # cost for one tile # same as intermittent power, but S=1
...
def est_cost_CONV_layercomp_contpow(layer, params_exec, plat_cost_profile):
    ...
    params_pres = {'backup_batch_size': 1}  # S=1
    tile_en_cost, tile_lat_cost = est_cost_CONV_tilecomp_intpow(params_exec, params_pres,
                                                                plat_cost_profile)  # cost for one tile # same as intermittent power, but S=1
...
def est_cost_CONV_powcycle_intpow(params_exec, params_pres, plat_settings, plat_cost_profile, exclude_rb_cost=False,
                                  return_breakdown=False):
    E_rb, L_rb = est_cost_CONV_reboot_intpow(plat_cost_profile)
    E_fd_ngt0, L_fd_ngt0, E_fd_n0, L_fd_n0 = est_cost_CONV_tileinputfetch_intpow(params_exec, params_pres,
                                                                                 plat_cost_profile)
...
def est_cost_CONV_tileoutputbackup_intpow(params_exec, params_pres, plat_cost_profile):
    ...
    # nO, blkO = _num_datatrcmds_backup_tile_data(params_exec)
    # energy/latency cost of the transfer (for a given block size)
    ew_Tm = plat_cost_profile['E_DMA_VM_TO_NVM'](Tm);
    lw_Tm = plat_cost_profile['L_DMA_VM_TO_NVM'](Tm)
    ew_STm = plat_cost_profile['E_DMA_VM_TO_NVM'](S * Tm);
    lw_STm = plat_cost_profile['L_DMA_VM_TO_NVM'](S * Tm)
    # energy/latency overhead of each transfer
    eobO_ruI = plat_cost_profile['E_BD_O_RUI_OVHD'];
    lobO_ruI = plat_cost_profile['L_BD_O_RUI_OVHD']
    eobO_ruW = plat_cost_profile['E_BD_O_RUW_OVHD'];
    lobO_ruW = plat_cost_profile['L_BD_O_RUW_OVHD']
    eobO_ruO = plat_cost_profile['E_BD_O_RUO_OVHD'];
    lobO_ruO = plat_cost_profile['L_BD_O_RUO_OVHD']
