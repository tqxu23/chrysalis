...
from utils import *
import models.components.insitu.GammaCostCore.src.GAMMA.gamma as gamma
...
def train_model(model_defs, input_arg, map_cstr=None, chkpt_file='./chkpt', inter=False):
    global opt
    opt = input_arg
    ...
    env = gamma.GAMMA(dimension=dimension, num_pe=opt.num_pe, fitness=fitness, par_RS=opt.parRS,
                      ...
                      fixedCluster=opt.fixedCluster, log_level=opt.log_level, map_cstr=map_cstr, inter=inter)
    ...
    for dimension in model_defs:
        ...
        ans = {"Mapping": chkpt["best_sol"],"Energy":best_energy, "Reward": chkpt['best_reward'][0], "Runtime": best_runtime,
               "Area": best_area / 1e6, "PE Area_ratio": best_num_pe * MAC_AREA_INT8 / best_area * 100,
               "Num_PE": best_num_pe, "L1Buffer": best_l1_size, "L2Buffer": best_l2_size}
        return ans
        # chkpt = {
        ...
        # }
        # columns = ["runtime", "area", "pe_area_ratio", "PE", "L1_size", "L2_size", "PE_area", "L1_area", "L2_area",
        #            "best_sol"]
